...
function pack() {
  build
  version=$(./$app -v)
  tar zcvf $app-$version.tar.gz control cfg.example.json $app conf static views scripts
}
...
function reload() {
  curl -s 127.0.0.1:2001/config/reload | python -m json.tool
}
